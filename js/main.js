...
$(document).ready(function () {
  function getTweet() {
    fetch(
      'https://etdnpjkv4j.execute-api.ap-northeast-1.amazonaws.com/timeline/shotaro'
    )
      .then(res => {
        return res.json();
      })
  ...
  function getSpreadsheet() {
    ...
    fetch(
      'https://script.google.com/macros/s/AKfycbxZBcaRh3K27U5lEuqkmEx3uD1g9gO2cP5W7YSGNa8BP6vOe8Q/exec'
    )
      .then(res => {
        return res.json();
      })
  ...
  function output(data) {
    data.forEach(el => {
      html += `<aside class="tweet-card">
                  ...
                  <small class="date">posted at :${new Date(
                    el.created_at
                  )}</small>
                  </main>
                  </aside>`;
